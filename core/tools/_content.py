...
async def _content_alias_file(file_path: str, persistent: bool = False) -> ToolResult:
    alias_id = secrets.token_hex(4)
    abs_path = os.path.abspath(file_path)
    try:
        rel = os.path.relpath(abs_path, str(_state._engine.project_dir))
        if not rel.startswith(".."):
            _state._engine.add_content_alias(alias_id, {"type": "file", "path": rel, "relative": True, "persistent": persistent})
        else:
            _state._engine.add_content_alias(alias_id, {"type": "file", "path": abs_path, "persistent": persistent})
    except ValueError:
        _state._engine.add_content_alias(alias_id, {"type": "file", "path": abs_path, "persistent": persistent})
...
async def content_alias_file(file_path: str, persistent: bool = False) -> str:
    ...
    Args:
        ...
        persistent: If True, alias survives server restart. Default False.
    """
...
async def _content_alias_url(url: str, persistent: bool = False) -> ToolResult:
    alias_id = secrets.token_hex(4)
    _state._engine.add_content_alias(alias_id, {"type": "url", "url": url, "persistent": persistent})
    return ToolResult(data={"alias_path": f"/content/{alias_id}", "alias_id": alias_id})


@register_tool("content_alias_url")
async def content_alias_url(url: str, persistent: bool = False) -> str:
...
async def _get_aliases() -> ToolResult:
    ...
    for alias_id, entry in aliases.items():
        info = {"alias_id": alias_id, "alias_path": f"/content/{alias_id}", "type": entry["type"], "persistent": entry.get("persistent", False)}
        if entry["type"] == "file":
