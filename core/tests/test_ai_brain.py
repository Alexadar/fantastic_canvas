...
async def test_ensure_provider_no_config_returns_none(project_dir):
    """No config → no auto-discover → returns None."""
    brain = AIBrain(project_dir)
    provider = await brain.ensure_provider()
...
async def test_status_unconfigured(project_dir):
    ...
    assert status["provider_config"] is None
...
async def test_status_configured(project_dir):
    ...
    assert status["provider_config"]["model"] == "llama3.2"
    assert status["provider_config"]["endpoint"] == "http://localhost:11434"
