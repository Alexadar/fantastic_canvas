...
Lives as its own agent so cwd, timeout, and (future) sandboxing are
addressable per instance.
"""
...
async def _reflect(id, payload, kernel):
    """Identity + cwd + count of in-flight subprocesses. No args."""
...
async def _exec(id, payload, kernel):
    """args: code:str (req), timeout:float? (default 60), cwd:str? (overrides agent cwd). Spawns `python -c code`, captures pipes. Returns {stdout, stderr, exit_code, timed_out:bool}."""
    code = payload.get("code", "")
    if not isinstance(code, str) or not code:
        return {"error": "python_runtime: code (str) required"}
    timeout = float(payload.get("timeout", 60))
    cwd = payload.get("cwd") or (kernel.get(id) or {}).get("cwd") or None
