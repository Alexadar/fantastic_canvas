...
async def test_history_returns_messages(seeded_kernel, file_agent, tmp_path):
    ...
    chat = json.dumps([{"role": "user", "content": "hi"}, {"role": "assistant", "content": "hello"}])
    await seeded_kernel.send(
        file_agent,
        {"type": "write", "path": f".fantastic/agents/{oid}/chat_cli.json", "content": chat},
    )
    r = await seeded_kernel.send(oid, {"type": "history"})
...
async def test_history_per_client(seeded_kernel, file_agent, tmp_path):
    ...
    for cid, last in (("alice", "A-reply"), ("bob", "B-reply")):
        ...
        await seeded_kernel.send(
            file_agent,
            {"type": "write", "path": f".fantastic/agents/{oid}/chat_{cid}.json", "content": chat},
        )
...
async def test_run_persists_per_client_threads(seeded_kernel, file_agent, tmp_path):
    ...
    ot._providers[oid] = fp
    try:
        await seeded_kernel.send(oid, {"type": "send", "text": "hi A", "client_id": "alice"})
        await seeded_kernel.send(oid, {"type": "send", "text": "hi B", "client_id": "bob"})
...
async def test_cli_caller_routes_to_cli_only(seeded_kernel, file_agent):
    ...
    try:
        with patch.object(seeded_kernel, "send", spy_send), \
...
async def test_browser_caller_routes_to_own_inbox_only(seeded_kernel, file_agent):
    ...
    try:
        ...
        own_tokens = [
            p for t, p in emits if t == oid and p.get("type") == "token"
        ]
        own_done = [
            p for t, p in emits if t == oid and p.get("type") == "done"
        ]
...
async def test_menu_invalidates_after_tool_call(seeded_kernel, file_agent):
    ...
    try:
        ...
        await seeded_kernel.send(oid, {"type": "send", "text": "list", "client_id": "alice"})
    ...
    try:
        with patch.object(seeded_kernel, "send", spy_send), \
            ...
            t1 = asyncio.create_task(
                seeded_kernel.send(oid, {"type": "send", "text": "first", "client_id": "alice"})
            )
            await asyncio.sleep(0.05)   # let t1 acquire the lock
            t2 = asyncio.create_task(
                seeded_kernel.send(oid, {"type": "send", "text": "second", "client_id": "bob"})
            )
        ...
        bob_queued = [
            p for kind, t, p in captured
            if kind == "emit" and t == oid
            and p.get("type") == "queued" and p.get("client_id") == "bob"
        ]
        assert len(bob_queued) == 1, f"expected exactly one queued event for bob; got {len(bob_queued)}"
        ...
        bob_tokens = [
            p for kind, t, p in captured
            if kind == "emit" and t == oid
            and p.get("type") == "token" and p.get("client_id") == "bob"
        ]
...
async def test_concurrent_sends_serialize_per_backend(seeded_kernel, file_agent):
    ...
    try:
        results = await asyncio.gather(
            seeded_kernel.send(oid, {"type": "send", "text": "first",  "client_id": "a"}),
            seeded_kernel.send(oid, {"type": "send", "text": "second", "client_id": "b"}),
        )
