...
the substrate primer (primitive, envelope, send_syntax, well_known
agents, `binary_protocol` and `browser_bus` fields). Any LLM CLI dropped
in cold can bootstrap from one reflect call.
...
- **Unit/integration via `pytest`** — fast, parallel, in-process. ~160 tests.
  ```bash
...
└── bundled_agents/
    ├── core/{pyproject.toml, src/core/, tests/, selftest.md}
    ├── cli/...
    ├── webapp/                               # HTTP+WS transport
    ├── scheduler/, file/
...
Every agent answers `{type:"reflect"}`. Returns `{sentence, …flat state}`.
Reflect on the kernel itself (`send("kernel", {reflect})`) returns the
substrate primer — the only thing an external tool needs to bootstrap.
