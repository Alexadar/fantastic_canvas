...
answers them natively for its own children. `kernel_state` IS the root
loader: the tree root is an `kernel_state` agent (`id="kernel_state"`,
`handler_module="kernel_state.tools"`) that owns `.fantastic/` and answers
...
assert d['id'] == 'kernel_state', f'id={d.get(\"id\")}'
...
assert isinstance(d['tree'], dict) and d['tree']['id'] == 'kernel_state'
...
import json, sys
d = json.load(sys.stdin)
assert isinstance(d['tree'], list) and 'kernel_state' in d['tree'], f'tree={d[\"tree\"]}'
print('tree=ids: PASS')
"
...
```bash
rm -rf .fantastic
ID=$(fantastic kernel_state create_agent handler_module=file_bridge.tools description="x" | python -c "import json,sys;print(json.load(sys.stdin)['id'])")
...
```
Expected: `{"agents":[…]}` containing `kernel_state` (root). If the
...
disk. Non-tty invocations (pipes, scripted) see only `kernel_state`.

### Test 3: create_agent + auto-boot

```bash
fantastic kernel_state create_agent handler_module=file_bridge.tools
```
Expected: `{"id":"file_bridge_<hex6>", "handler_module":"file_bridge.tools",
"parent_id":"kernel_state"}`. File check:
`.fantastic/agents/file_bridge_<hex6>/agent.json` exists.

### Test 4: update_agent persists + emits agent_updated

```bash
ID=$(fantastic kernel_state list_agents | python -c "import json,sys;print([a for a in json.load(sys.stdin)['agents'] if a['handler_module']=='file_bridge.tools'][0]['id'])")
fantastic kernel_state update_agent id=$ID model=foo x=42
```
Expected: `{"updated":true,"id":"…","agent":{… "model":"foo", "x":42}}`.
File check: `agent.json` of that id contains `"model":"foo"` and
`"x":42`. Watchers of `kernel_state` see an `agent_updated` emit (verified
in pytest; fragile to drive via CLI).

### Test 5: delete_agent + cascade through subtree

```bash
fantastic kernel_state delete_agent id=$ID
```
...
```bash
echo "@kernel_state list_agents" | fantastic
```
...
```bash
rm -rf .fantastic
ID=$(fantastic kernel_state create_agent handler_module=file_bridge.tools delete_lock=true | python -c "import json,sys;print(json.load(sys.stdin)['id'])")
...
fantastic kernel_state delete_agent id=$ID | python -c "
import json, sys
d = json.load(sys.stdin)
...
fantastic kernel_state update_agent id=$ID delete_lock=false >/dev/null
fantastic kernel_state delete_agent id=$ID | python -c "
import json, sys
d = json.load(sys.stdin)
...
(kernel_state) rmtrees the dir on the `removed` state event. Bundles that
...
ws_bridge cancels its read loop).

```bash
rm -rf .fantastic
ID=$(fantastic kernel_state create_agent handler_module=file_bridge.tools | python -c "import json,sys;print(json.load(sys.stdin)['id'])")
# file_bridge.tools has no on_delete — the loader rmtrees the dir on `removed`.
fantastic kernel_state delete_agent id=$ID | python -c "
import json, sys
d = json.load(sys.stdin)
...
fantastic kernel_state delete_agent id=$PARENT | python -c "
import json, sys
d = json.load(sys.stdin)
...
fantastic kernel_state '<|"|list_agents<|"|' 2>&1 | python -c "
import sys
...
`kernel_state` ships a `readme.md`; the bootstrap copies it to
...
the bootstrap's root-readme seed not wired, or `kernel_state` ships no
`readme.md`.
