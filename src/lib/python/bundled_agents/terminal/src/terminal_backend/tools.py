...
shell naturally throttles. A streaming consumer (terminal_view (the TS frontend)) acks
...
def _clear_flow_control(agent_id: str, kernel) -> None:
    ...
    fresh streaming consumer (terminal_view (the TS frontend) mounting → `boot`) or a
...
async def _ack(id, payload, kernel):
    """args: chars:int. Flow-control ack from a streaming consumer (terminal_view (the TS frontend) acks each chunk after xterm parses it). Decrements the unacked-char count; re-attaches the PTY reader once it drops below LOW_WATERMARK. Returns {unacked:int, paused:bool}."""
    state = _procs.get(id)
    if not state:
        return {"error": "not running"}
