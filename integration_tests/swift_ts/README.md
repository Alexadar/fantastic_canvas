...
- `../test_decoupling_bundle_catalog.py` — the swift catalog no longer
  registers the 7 view bundles.
- `../test_serve_frontend.py` — the swift binary serves a static `dist/`
...
Prereqs to run: `cd swift && swift build`, `cd ts && npm run build`, system
