"""Bridge integration test — Python ↔ Python over WS (asymmetric).

Baseline harness validator for the WS-only kernel_bridge. A's bridge
opens a WS to B's `web_ws` surface and ships raw call frames; B's
`web_ws` dispatches `kernel.send(target, payload)` exactly like a
browser frame. **No B-side bridge agent needed.**

Topology:

    Workdir A (client)        Workdir B (server)
    ──────────────────        ──────────────────
    core                      core
    web (port_a)              web (port_b)
    web_ws                    web_ws            ← serves /<id>/ws
    bridge (transport=ws) ──ws──▶ ws://127.0.0.1:port_b/core/ws
        forward(target=core, payload) → {type:call,target:core,payload}
                                      ↓
                             B.web_ws._on_call → kernel.send(core, payload)
                                      ↓
                             reply rides back over the same socket

Ordering: spawn B first (the bridge connects eagerly at boot, so the
server must be up). After both are ready we call `boot` once — it
returns {already:true} if the daemon already connected, or connects
now if its startup boot raced B.
"""

from __future__ import annotations

import pytest

from helpers.seeding import seed_bridge_ws, seed_web, seed_web_ws


async def _bring_up(python_binary, python_kernel, parity_tmp, free_port, tag):
    base = parity_tmp(tag)
    workdir_a = base / "A"
    workdir_b = base / "B"
    workdir_a.mkdir(parents=True)
    workdir_b.mkdir(parents=True)

    port_a = free_port()
    port_b = free_port()

    # Both sides: web + web_ws (so each is drivable + serves /<id>/ws).
    for wd, port in [(workdir_a, port_a), (workdir_b, port_b)]:
        seed_web(python_binary, wd, port)
        seed_web_ws(python_binary, wd)

    # Only A gets a bridge. peer_id=core → dial ws://B/core/ws.
    seed_bridge_ws(
        python_binary, workdir_a,
        agent_id="bridge",
        peer_id="core",
        peer_port=port_b,
    )

    # Spawn B (server) first, then A (client). A's daemon boots the
    # bridge → connects to the already-up B.
    kernel_b = await python_kernel(workdir_b, port_b)
    kernel_a = await python_kernel(workdir_a, port_a)

    # Idempotent connect guard — no-op if already connected.
    await kernel_a.call("bridge", "boot")
    return kernel_a, kernel_b


@pytest.mark.asyncio
async def test_python_python_ws_forward_reflect(
    python_binary, python_kernel, parity_tmp, free_port
):
    kernel_a, _ = await _bring_up(
        python_binary, python_kernel, parity_tmp, free_port, "py_py_ws_reflect"
    )

    reply = await kernel_a.call(
        "bridge", "forward",
        target="core",
        payload={"type": "reflect"},
    )

    assert isinstance(reply, dict), f"expected dict, got {type(reply)}: {reply}"
    assert "error" not in reply, f"forward returned error: {reply}"
    # Python's primer reflect carries `sentence` + `tree` keys.
    assert "sentence" in reply or "tree" in reply, (
        f"reply lacks primer fields: {list(reply.keys())}"
    )


@pytest.mark.asyncio
async def test_python_python_ws_forward_list_agents(
    python_binary, python_kernel, parity_tmp, free_port
):
    kernel_a, _ = await _bring_up(
        python_binary, python_kernel, parity_tmp, free_port, "py_py_ws_list"
    )

    reply = await kernel_a.call(
        "bridge", "forward",
        target="core",
        payload={"type": "list_agents"},
    )

    assert isinstance(reply, dict), f"expected dict, got {reply}"
    assert "agents" in reply, f"list_agents reply missing 'agents': {reply}"
    ids = {a.get("id") for a in reply["agents"] if isinstance(a, dict)}
    # B was seeded with: core, web, web_ws.
    assert "core" in ids, f"B's list_agents should include core, got {ids}"
    assert "web" in ids, f"B's list_agents should include web, got {ids}"
