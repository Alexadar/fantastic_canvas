...
### Test 1: WS handshake to /kernel_state/ws succeeds
    ...
    async with websockets.connect('ws://localhost:$PORT/kernel_state/ws') as ws:
        ...
        for _ in range(5):
            ...
            if m.get('id')=='1' and m.get('type')=='reply':
                ...
                ok = d.get('id') == 'kernel_state' and 'tree' in d and 'transports' not in d
...
### Test 4: delete web_ws → /kernel_state/ws stops handshaking
...
async def main():
    async with websockets.connect('ws://localhost:$PORT/kernel_state/ws') as ws:
        await ws.send(json.dumps({'type':'call','target':'kernel_state','payload':{'type':'delete_agent','id':'$WSID'},'id':'1'}))
        while True:
            m = json.loads(await ws.recv())
...
async def main():
    try:
        await websockets.connect('ws://localhost:$PORT/kernel_state/ws')
