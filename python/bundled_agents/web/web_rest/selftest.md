...
# on the web agent (not fs_loader) so the new agents land under web's dir.
...
### Test 1: POST /<rest_id>/fs_loader body={type:reflect} returns fs_loader's reflect

```bash
curl -s -X POST -H 'content-type: application/json' \
  -d '{"type":"reflect"}' http://localhost:$PORT/$RID/fs_loader | python -c "
import json, sys
...
### Test 2: POST /<rest_id>/fs_loader body={type:list_agents} returns agents

```bash
curl -s -X POST -H 'content-type: application/json' \
  -d '{"type":"list_agents"}' http://localhost:$PORT/$RID/fs_loader | python -m json.tool | head -10
```
Expected: JSON with `agents:[…]` containing fs_loader, the web agent, the
web_rest child.
...
curl -s -o /dev/null -w '%{http_code}\n' -X POST \
  ...
  http://localhost:$PORT/$RID/fs_loader
```
Expected: `400`.
...
async def main():
    async with websockets.connect('ws://localhost:$PORT/fs_loader/ws') as ws:
        await ws.send(json.dumps({'type':'call','target':'fs_loader','payload':{'type':'create_agent','handler_module':'web_rest.tools','parent_id':'$WEB_ID'},'id':'1'}))
        while True:
...
curl -s -X POST -d '{"type":"reflect"}' http://localhost:$PORT/$RID/fs_loader | python -c "import json,sys;print('rid1' if json.load(sys.stdin).get('id')=='fs_loader' else 'FAIL')"
curl -s -X POST -d '{"type":"reflect"}' http://localhost:$PORT/$RID2/fs_loader | python -c "import json,sys;print('rid2' if json.load(sys.stdin).get('id')=='fs_loader' else 'FAIL')"
```
...
```bash
curl -s http://localhost:$PORT/$RID/_reflect/fs_loader | python -c "
import json, sys
d = json.load(sys.stdin)
# fs_loader reflect is the root agent's uniform identity + tree.
assert d.get('id') == 'fs_loader', f'id={d.get(\"id\")}'
assert 'tree' in d
...
async def main():
    async with websockets.connect('ws://localhost:$PORT/fs_loader/ws') as ws:
        await ws.send(json.dumps({'type':'call','target':'fs_loader','payload':{'type':'delete_agent','id':'$RID'},'id':'1'}))
        while True:
            m = json.loads(await ws.recv())
            if m.get('id')=='1': break
asyncio.run(main())
"
sleep 0.3
code=$(curl -s -o /dev/null -w '%{http_code}' -X POST -d '{"type":"reflect"}' http://localhost:$PORT/$RID/fs_loader)
...
| # | Test | Pass |
|---|------|------|
| 1 | POST → fs_loader reflect (identity + tree) | |
| 2 | POST → list_agents | |
| 3 | bad JSON → 400 | |
