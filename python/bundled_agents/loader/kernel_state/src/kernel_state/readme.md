...
### Durable state: wire a persistence provider (or stay in RAM)

The root (`kernel_state`) auto-persists records ONLY through a stream provider it
**discovers**: the first `file_bridge` child of the root whose `root` resolves to
`.fantastic`. **No provider ⇒ no persistence — the tree lives in RAM** (lost on
restart). There is no direct-write fallback (this is a kernel, not a SaaS — one
path, no hidden second route). Wire one to make the kernel durable:
...
(readme-seeding too). It's a normal, visible, re-gateable agent — reflect it like
any other. Point it at a `network_bridge` instead and state persists to a remote
