...
`python_runtime`: a scheduler, a UI button, a python routine, or another AI can
...
Given those, route your own output: `send()` to a host listener, or simply answer
— a frontend listener `watch`es your id and consumes your stream. There is no
...
Every routine (this AI, a host python job, or a browser `html_agent`'s JS) reaches
every agent by id. From any of them you can read memory anywhere (`send(<state>,
