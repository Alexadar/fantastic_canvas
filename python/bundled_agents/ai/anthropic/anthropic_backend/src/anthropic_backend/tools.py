...
def _get_provider(id: str, kernel):
    if id not in _providers:
        ...
        _providers[id] = AnthropicProvider(endpoint=endpoint, model=model)
...
async def _send(id, payload, kernel):
    ...
    if id in call_stack:
        return {
            "error": "anthropic_backend: cycle detected",
            "response": "",
            "cycle": call_stack + [id],
            "client_id": client_id,
        }
    if len(call_stack) >= MAX_CALL_DEPTH:
        return {
            "error": f"anthropic_backend: max call depth {MAX_CALL_DEPTH} reached",
            "response": "",
            "client_id": client_id,
        }
    ...
    if not stateless and not _file_agent_id(id, kernel):
        return {"error": "anthropic_backend: file_agent_id required"}
...
async def handler(id: str, payload: dict, kernel) -> dict | None:
    ...
    fn = VERBS.get(t)
    if fn is None:
        return {"error": f"anthropic: unknown type {t!r}"}
