...
# ─── the injected kernel connector (prepended to every job's code) ───────────
#
# A plain-Python IIFE-ish preamble (no imports leaking past `_`-names) that wraps
# the inherited control fd, spawns a daemon reader thread to demux inbound frames
# (reply/event/inbox), and exposes a kernel-mirroring `kernel` global. Frames are
# length-prefixed JSON (4-byte BE length + body) — same convention as the WS
# binary frame header. If the channel can't be set up, `kernel` degrades to a stub
# that raises a clear error, so a job that doesn't touch the kernel still runs.

_CONNECTOR_SRC = r"""# --- fantastic kernel connector (injected; talks to spawner over control fd) ---
try:
