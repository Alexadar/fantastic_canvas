...
(`allow_all` / `deny_inbound`) map to `Silent` — they present nothing — so the
legacy `auth` shorthand stays consistent (`auth:"deny_inbound"` ⇒ presents nothing,
`auth:"password"` ⇒ presents the group token).
"""

from __future__ import annotations

from bridge_core._authorizer import EgressRule, construct, parse_spec
from bridge_core.egress_rules.password import Password
from bridge_core.egress_rules.silent import Silent
...
def resolve_egress(record: dict) -> EgressRule:
    """Resolve the leg's EGRESS rule: `egress_rule` if the record carries it, else
    the legacy `auth` shorthand (so `auth:"password"` presents the group token).
    Absent ⇒ `Silent` (back-compat — present nothing). Unknown type ⇒ `ValueError`."""
