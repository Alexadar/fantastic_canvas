...
//! # Frame protocol
//!
//! Matches Python's `kernel_bridge._transport` byte-for-byte, which
//! in turn matches `fantastic-web`'s WS server frame shape:
//!
//! ```text
//! outbound  {type:"call",  target:peer_id,
//!            payload:{type:"forward", target, payload, corr_id},
//!            id:corr_id}
//! inbound   {type:"call",  ...}           — read loop unwraps + kernel.send
...
fn reflect_reply(agent_id: &AgentId, kernel: &Kernel) -> Value {
    ...
    json!({
        ...
        "verbs": {
            ...
            "forward": "args: target:str, payload:dict, timeout:float? (default 30s). Ships the inner payload to the peer bridge, awaits the reply, returns it unwrapped.",
        },
        "emits": {
...
async fn boot_reply(agent_id: &AgentId, kernel: &Arc<Kernel>) -> Value {
    ...
    let transport: Arc<dyn BridgeTransport> = match kind.as_str() {
        ...
        "ws" => {
            ...
            let port = match meta_u64(agent_id, kernel, "port") {
                Some(p) => p as u16,
                None => return json!({"error": "kernel_bridge: ws transport requires port"}),
            };
            let host =
        ...
        "http" => {
            let url = match meta_string(agent_id, kernel, "url") {
                Some(u) => u,
                None => {
                    return json!({
                        "error": "kernel_bridge: http transport requires `url` (e.g. http://host:port/<rest_id>/)"
                    })
                }
            };
...
async fn forward_reply(agent_id: &AgentId, payload: &Value, kernel: &Arc<Kernel>) -> Value {
    ...
    let peer_id = meta_string(agent_id, kernel, "peer_id").unwrap_or_default();
    ...
    loop {
        ...
        match ftype {
            "call" => {
                ...
                if payload.get("type").and_then(Value::as_str) != Some("forward") {
                    // Unknown call shape — reply with an error so the
                    // peer's pending oneshot resolves cleanly.
                    let err_reply = json!({
                        "type": "reply",
                        "id": corr_id,
                        "data": {
                            "error": format!(
                                "kernel_bridge: unknown call payload type {:?}",
                                payload.get("type").and_then(Value::as_str).unwrap_or("")
                            )
                        }
                    });
                    let _ = state.transport.send_frame(err_reply).await;
                    continue;
                }
                let target = payload
                    .get("target")
                ...
                let reply = if target.is_empty() {
                    json!({"error": "kernel_bridge: empty forward target"})
                } else {
